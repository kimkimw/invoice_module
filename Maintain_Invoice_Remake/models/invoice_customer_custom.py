...
from custom.Maintain_Tax.tax import get_product_tax_category
import logging
...
# Variable global for control line no
CONTROL_LINE_NO_FOLLOW_ORDER_BY = 'isResetLineNoFollowAsc'
partner_id_for_price_history =  -1
product_barcode_for_price_history =  ''

# store special business type
special_product_list = {"サンプル":{"barcode": {"value":"0000000000000","visible":False,"required":False}
                                    ,"product_code": {"value":"","visible":False,"required":False}
                                    ,"product_name": {"value":"サンプル","visible":True,"required":False}
                                    ,"calc_type":""
                                    ,"allow_copy":True
                                    ,"product_uom_qty": {"value":0,"visible":False,"required":False}
                                    ,"price_unit": {"value":0,"visible":False,"required":False}
                                    ,"tax_rate":{"value":0,"visible":False,"required":False}
                                    ,"product_maker_name": {"value":"","visible":False,"required":False}
                                    ,"supplier_code": {"value":"","visible":False,"required":False}
                                    ,"product_standard_number": {"value":"","visible":False,"required":False}
                                    ,"description":{"value":"","visible":False,"required":False}
                                    ,"product_uom_id":{"value":"","visible":False,"required":False}
                                    ,"line_amount":{"value":0,"visible":False,"required":False}
                                    ,"product_standard_price":{"value":0,"visible":False,"required":False}
                                    ,"cost": {"value":0,"visible":False,"required":False}
                                 },
                         "小計":{"barcode": {"value":"8888888888888","visible":False,"required":False}
                                 , "product_code": {"value": "", "visible": False,"required":False}
                                 , "product_name": {"value": "小計", "visible": True,"required":False}
                                 , "calc_type": "sum"
                                 , "allow_copy":False
                                 , "product_uom_qty": {"value": 0, "visible": False,"required":False}
                                 , "price_unit": {"value": 0, "visible": False,"required":False}
                                 ...
                                 , "description": {"value": "", "visible": False,"required":False}
                                 , "product_uom_id": {"value": "", "visible": False,"required":False}
                                 , "line_amount": {"value": 0, "visible": True,"required":False}
                                 , "product_standard_price": {"value": 0, "visible": False,"required":False}
                                 , "cost": {"value": 0, "visible": False,"required":False}
                                 },
                        "セクション": {"barcode": {"value": "7777777777777", "visible": False, "required": False}
                            , "product_code": {"value": "", "visible": False, "required": False}
                            , "product_name": {"value": "セクション", "visible": True, "required": False}
                            , "calc_type": "section"
                            , "allow_copy": False
                            , "product_uom_qty": {"value": 0, "visible": False, "required": False}
                            , "price_unit": {"value": 0, "visible": False, "required": False}
                        ...
                        "中計":{"barcode": {"value":"7777777777777","visible":False,"required":False}
                             , "product_code": {"value": "", "visible": False,"required":False}
                             , "product_name": {"value": "中計", "visible": True,"required":False}
                             , "calc_type": "subtotal"
                             , "allow_copy":False
                             , "product_uom_qty": {"value": 0, "visible": False,"required":False}
                             , "price_unit": {"value": 0, "visible": False,"required":False}
...
def rounding(number, pre=0, type_rounding='round'):
    ...
    if number != 0:
        number = float_round(number, pre + 3)
...
def copy_data_from_partner(rec, partner, quotation):
    if partner:
        ...
        if quotation:
            ...
            # MOD - LIEM - 20230929 - START
            # rec.x_voucher_tax_transfer = quotation.tax_method
            if quotation.tax_method and quotation.tax_method != 'foreign_tax':
                rec.x_voucher_tax_transfer = quotation.tax_method
            else:
                rec.x_voucher_tax_transfer = 'voucher'
            # MOD - LIEM - 20230929 - END
        else:
            # MOD - LIEM - 20230929 - START
            # rec.x_voucher_tax_transfer = get_tax_method(tax_unit=partner.customer_tax_unit)
            if partner.customer_tax_unit and partner.customer_tax_unit != 'foreign_tax':
                rec.x_voucher_tax_transfer = get_tax_method(tax_unit=partner.customer_tax_unit)
            else:
                rec.x_voucher_tax_transfer = 'voucher'
            # MOD - LIEM - 20230929 - END
...
def copy_data_from_history_quotation_partner(rec, partner, quotation):
    if partner:
        rec.partner_id = partner
        rec.x_studio_address_1 = partner.street
        rec.x_studio_address_2 = partner.street2
        rec.x_studio_address_3 = partner.address3
        # Long MOD START 2022-05-18
        # rec.sales_rep = partner.customer_agent
        # Long MOD END 2022-05-18
        rec.x_studio_payment_rule_1 = partner.payment_rule
        rec.x_studio_price_list = partner.property_product_pricelist
        rec.invoice_payment_terms_custom = partner.payment_terms
        rec.customer_tax_rounding = partner.customer_tax_rounding
        ...
        if quotation:
            rec.x_studio_summary = quotation.comment_apply
            rec.x_studio_name = quotation.partner_name
            rec.x_bussiness_partner_name_2 = quotation.partner_name_2
...
def copy_data_from_quotation(rec, quotation, account):
    global special_product_list
    new_quotation_custom_line_no = 0  #renew line_no
    if quotation:
        # rec.invoice_line_ids = ()
        invoice_line_ids = []
        line_ids = []

        rec.delivery_id = quotation.delivery_id
        rec.delivery_name = quotation.delivery_name
        rec.delivery_address = quotation.delivery_address
        ...
        for line in rec.trigger_quotation_history.order_line.sorted(lambda l: l.quotation_custom_line_no):
            if (line.class_item in special_product_list.keys() and special_product_list[line.class_item]["allow_copy"]) or (line.class_item not in special_product_list.keys()):
                if line.product_id:
                    invoice_line_ids.append((0, False, {
                        ...
                        'x_product_cost_price': line.cost,
                        'product_standard_price': line.product_standard_price,
                        ...
                        'product_tax_category': get_product_tax_category(line, line)
                    }))
...
class ClassInvoiceCustom(models.Model):
    ...
    # ===============================
    # ADD - LIEM - 20220105 - START
    # ===============================
    def check_date_less_than_bill_date(self, partner, invoice_date):

        if len(partner) == 1 and invoice_date:
            # self._cr.execute("""
            #     SELECT id FROM bill_info
            #     WHERE partner_id = %s
            #     AND   deadline >= %s
            # """, [vals.get('partner_id'), vals.get('x_studio_date_invoiced')])

            self._cr.execute("""
                SELECT id FROM bill_info
                WHERE partner_id = %s
                AND   deadline >= %s
            """, [partner.id, invoice_date])

            bill_info_ids = self._cr.fetchall()

            return len(bill_info_ids) > 0

        else:
            return False

    # ===============================
    # ADD - LIEM - 20220105 - END
    # ===============================
    ...
    # start add for gross profit TrungTM 23.04.22
    amount_cost = fields.Monetary(string='amount cost', store=True, readonly=True)
    amount_gross_profit = fields.Monetary(string='amount gross profit', store=True, readonly=True)
    gross_profit_rate = fields.Float(string='Untaxed amount_cost', store=True, digits=(12, 3), readonly=True)
    profit_management_flg = fields.Boolean('profit management flg', default=False, compute='_check_flag_history')
    # start add for gross profit TrungTM 23.04.22
    ...
    x_voucher_tax_transfer = fields.Selection([
        ...
        ('internal_tax', '内税／明細'),
        ('custom_tax', '税調整別途')
    ], string='Tax Transfer', default='voucher')
    ...
    delivery_id = fields.Many2one('res.delivery', string='納入先コード')
    delivery_code = fields.Char(string='Delivery Code')
    delivery_name = fields.Text(string='Delivery Name')
    delivery_address = fields.Text(string='Delivery Address')

    partner_department = fields.Many2one('hr.department', required=True)

    @api.onchange('x_studio_business_partner', 'sales_rep')
    def _onchange_department(self):
        self.partner_department = self.sales_rep.employee_section_id.department_code1
    ...
    def copy_from_history(self):
        ...
        # Start Upd 23/06.15 TrungTM
        # # LongDN MOD START 2022/04/27
        # # Reset invoice_line_custom follow Asc or Desc AND find isLastInvoiceCustomLineNo
        # if len(self.invoice_line_ids) > 0:
        #     if CONTROL_LINE_NO_FOLLOW_ORDER_BY == 'isResetLineNoFollowAsc':
        #         for index, invoice_line_id in enumerate(self.invoice_line_ids):
        #             invoice_line_id.invoice_custom_line_no = index + 1
        #         isLastInvoiceCustomLineNo = self.invoice_line_ids[-1].invoice_custom_line_no
        #     elif CONTROL_LINE_NO_FOLLOW_ORDER_BY == 'isResetLineNoFollowDesc':
        #         for index, invoice_line_id in enumerate(self.invoice_line_ids):
        #             invoice_line_id.invoice_custom_line_no = self.invoice_line_ids[0].invoice_custom_line_no - index
        #         isLastInvoiceCustomLineNo = self.invoice_line_ids[0].invoice_custom_line_no
        #     elif CONTROL_LINE_NO_FOLLOW_ORDER_BY == 'doNotNeedResetLineNo':
        #         isLastInvoiceCustomLineNo = len(self.invoice_line_ids)
        # else:
        #     isLastInvoiceCustomLineNo = 0
        # # LongDN MOD END 2022/04/27
        isFirstInvoiceCustomLineNo = 0
        isLastInvoiceCustomLineNo = 0
        if len(self.invoice_line_ids) > 0:
            isLastInvoiceCustomLineNo = self.invoice_line_ids[0].invoice_custom_line_no
            isFirstInvoiceCustomLineNo = self.invoice_line_ids[0].invoice_custom_line_no
        for line in self.invoice_line_ids:
            if line.invoice_custom_line_no <= isFirstInvoiceCustomLineNo:
                isFirstInvoiceCustomLineNo = line.invoice_custom_line_no
            if line.invoice_custom_line_no >= isLastInvoiceCustomLineNo:
                isLastInvoiceCustomLineNo = line.invoice_custom_line_no
        # End Upd 23/06.15 TrungTM

        if self.copy_history_from == 'sale.order.line' and self.copy_history_item:
            product_ids = self.copy_history_item.split(',')
            invoice_line_vals = []
            for product_id in product_ids:
                product = self.env["sale.order.line"].search([('id', '=', int(product_id))])
                if product:
                    invoice_line_vals.append((0, False, {
                        'x_invoicelinetype': product.class_item,
                        'product_id': product.product_id,
                        'product_code': product.product_code,
                        'product_barcode': product.product_barcode,
                        'product_name': product.product_name,
                        'product_name2': product.product_name2,
                        'invoice_custom_standardnumber': product.product_standard_number,
                        ...
                        'product_standard_price': product.product_standard_price,
                        'product_uom_id': product.product_uom_id,
                        'invoice_custom_lineamount': product.line_amount,
                        'invoice_custom_lineamount_cost': product.line_amount_cost,
                        'tax_rate': product.tax_rate,
                        'line_tax_amount': product.line_tax_amount,
                        ...
                        'invoice_custom_Description': product.description,
                        # LONG MOD START 2022/04/18
                        # Change len(self.invoice_line_ids) + 1 = isLastInvoiceCustomLineNoForSaleOrderLine + 1
                        # 'invoice_custom_line_no': len(self.invoice_line_ids) + 1,
                        'invoice_custom_line_no': isLastInvoiceCustomLineNo + 1,
                        # LONG MOD END 2022/04/18
                        'copy_history_flag': True,
                        'product_tax_category': get_product_tax_category(product, product),
                    }))
                    isLastInvoiceCustomLineNo += 1
            self.invoice_line_ids = invoice_line_vals
        elif self.copy_history_from == 'account.move.line' and self.copy_history_item:
            product_ids = self.copy_history_item.split(',')
            invoice_line_vals = []
            for product_id in product_ids:
                product = self.env["account.move.line"].search([('id', '=', int(product_id))])
                if product:
                    invoice_line_vals.append((0, False, {
                        'x_invoicelinetype': product.x_invoicelinetype,
                        'product_id': product.product_id,
                        'product_code': product.product_code,
                        'product_barcode': product.product_barcode,
                        'product_name': product.product_name,
                        'product_name2': product.product_name2,
                        'invoice_custom_standardnumber': product.invoice_custom_standardnumber,
                        'product_maker_name': product.product_maker_name,
                        'supplier_code': product.supplier_code,
                        'quantity': product.quantity,
                        'price_unit': product.price_unit,
                        'x_product_cost_price': product.x_product_cost_price,
                        'product_standard_price': product.product_standard_price,
                        'product_uom_id': product.product_uom_id,
                        'invoice_custom_lineamount': product.invoice_custom_lineamount,
                        # Start Add for amount cost TrungTM 23.04.23
                        'invoice_custom_lineamount_cost': product.invoice_custom_lineamount_cost,
                        # End Add for amount cost TrungTM 23.04.23
                        'tax_rate': product.tax_rate,
                        'line_tax_amount': product.line_tax_amount,
                        'account_id': self.env.company.get_chart_of_accounts_or_fail().id,
                        'price_include_tax': product.price_include_tax,
                        'price_no_tax': product.price_no_tax,
                        'invoice_custom_Description': product.invoice_custom_Description,
                        # LONG MOD START 2022/04/18
                        # Change len(self.invoice_line_ids) + 1 = isLastInvoiceCustomLineNoForAccountMoveLine + 1
                        # 'invoice_custom_line_no': len(self.invoice_line_ids) + 1,
                        'invoice_custom_line_no': isLastInvoiceCustomLineNo + 1,
                        # LONG MOD END 2022/04/18
                        'copy_history_flag': True,
            ...
            for product_id in product_ids:
                ...
                if product:
                    invoice_line_vals.append((0, False, {
                        ...
                        'invoice_custom_standardnumber': product.product_custom_standardnumber,
                        'product_maker_name': product.product_maker_name,
                        'supplier_code': product.supplier_code,
                        ...
                        'account_id': self.env.company.get_chart_of_accounts_or_fail().id,
                        # LONG MOD START 2022/04/18
                        # Change len(self.invoice_line_ids) + 1 = isLastInvoiceCustomLineNoForProductProduct + 1
                        'invoice_custom_line_no': isLastInvoiceCustomLineNo + 1,
        ...
        elif self.copy_history_from == 'add':
            current = self.invoice_line_ids[int(self.copy_history_item)].invoice_custom_line_no
            for line in self.invoice_line_ids:
                if line.invoice_custom_line_no == current + 1:
                    for line_sup in self.invoice_line_ids:
                        if line_sup.invoice_custom_line_no > current:
                            line_sup.invoice_custom_line_no += 1
            self.invoice_line_ids = [(0, False, {
                'product_barcode': ' ',
                'invoice_custom_line_no': current + 1,
            })]
            self.invoice_line_ids = self.invoice_line_ids.sorted(key=lambda line: int(line.invoice_custom_line_no),
                                                                 reverse=False)
        elif self.copy_history_from == 'go_to':
            begin = self.invoice_line_ids[int(self.copy_history_item)].invoice_custom_line_no
            goto = int(self.invoice_line_ids[int(self.copy_history_item)].line_no_input)
            if goto < 1 or goto > isLastInvoiceCustomLineNo:
                self.invoice_line_ids[int(self.copy_history_item)].line_no_input = ''
            else:
                if (begin == 1 and goto == begin) or (begin == isLastInvoiceCustomLineNo and goto == begin):
                    self.invoice_line_ids[int(self.copy_history_item)].line_no_input = ''
                elif goto == isLastInvoiceCustomLineNo:
                    for line in self.invoice_line_ids:
                        if line.invoice_custom_line_no > begin and line.invoice_custom_line_no <= goto:
                            line.invoice_custom_line_no -= 1
                elif goto < begin:
                    for line in self.invoice_line_ids:
                        if line.invoice_custom_line_no == goto:
                            for line_sup in self.invoice_line_ids:
                                if line_sup.invoice_custom_line_no < begin and line_sup.invoice_custom_line_no >= goto:
                                    line_sup.invoice_custom_line_no += 1
                            break
                elif goto > begin:
                    for line in self.invoice_line_ids:
                        if line.invoice_custom_line_no == goto:
                            for line_sup in self.invoice_line_ids:
                                if line_sup.invoice_custom_line_no > begin and line_sup.invoice_custom_line_no <= goto:
                                    line_sup.invoice_custom_line_no -= 1
                            break
                self.invoice_line_ids[int(self.copy_history_item)].invoice_custom_line_no = goto
                self.invoice_line_ids[int(self.copy_history_item)].line_no_input = ''
            ...
            self.invoice_line_ids = [(0, False, {
                ...
                'invoice_custom_standardnumber': self.invoice_line_ids[
                    int(self.copy_history_item)].invoice_custom_standardnumber,
                'product_maker_name': self.invoice_line_ids[int(self.copy_history_item)].product_maker_name,
                'supplier_code': self.invoice_line_ids[int(self.copy_history_item)].supplier_code,
                ...
                'x_product_cost_price': self.invoice_line_ids[int(self.copy_history_item)].x_product_cost_price,
                'product_standard_price': self.invoice_line_ids[int(self.copy_history_item)].product_standard_price,
                'product_uom_id': self.invoice_line_ids[int(self.copy_history_item)].product_uom_id,
                'invoice_custom_lineamount': self.invoice_line_ids[
                    int(self.copy_history_item)].invoice_custom_lineamount,
                # Start Add for amount cost TrungTM 23.04.2023
                'invoice_custom_lineamount_cost': self.invoice_line_ids[
                    int(self.copy_history_item)].invoice_custom_lineamount_cost,
                # End Add for amount cost TrungTM 23.04.2023
                ...
                'invoice_custom_Description': self.invoice_line_ids[
                    int(self.copy_history_item)].invoice_custom_Description,
                # LONG MOD START 2022/04/18
                # Change len(self.invoice_line_ids) + 1 = isLastInvoiceCustomLineNoForDuplicated + 1
                # 'invoice_custom_line_no': len(self.invoice_line_ids) + 1,
                'invoice_custom_line_no': isLastInvoiceCustomLineNo + 1,
                # LONG MOD END 2022/04/18
                'copy_history_flag': self.invoice_line_ids[int(self.copy_history_item)].copy_history_flag,
                'product_tax_category': get_product_tax_category(self.invoice_line_ids[int(self.copy_history_item)],
                                                                 self.invoice_line_ids[int(self.copy_history_item)]),
            })]
        elif self.copy_history_from == 'multi_duplicated':
            copy_ids = self.copy_history_item.split(',')
            for copy_id in copy_ids:
                for line in self.invoice_line_ids:
                    # line_str = str(line)  # Convert the line object to a string
                    # start_index = line_str.find("=") + 1  # Find the index of '=' and add 1 to start after '='
                    # end_index = line_str.find(">", start_index)  # Find the index of '>' after the start_index
                    # if start_index != -1 and end_index != -1:  # Check if '=' and '>' are found
                    #     data_between_equal_and_greater = line_str[start_index:end_index].strip()  # Extract and strip the data
                    #     data_without_quotes = data_between_equal_and_greater.replace("'", "")  # Remove single quotes
                    if line.invoice_custom_line_no == int(copy_id):
                        self.invoice_line_ids = [(0, False, {
                            ...
                            'price_unit': line.price_unit,
                            'x_product_cost_price': line.x_product_cost_price,
                            'product_standard_price': line.product_standard_price,
                            'product_uom_id': line.product_uom_id,
                            'invoice_custom_lineamount': line.invoice_custom_lineamount,
                            'invoice_custom_lineamount_cost': line.invoice_custom_lineamount_cost,
                            'tax_rate': line.tax_rate,
                            'account_id': self.env.company.get_chart_of_accounts_or_fail().id,
                            'line_tax_amount': line.line_tax_amount,
                            'price_include_tax': line.price_include_tax,
                            ...
                            'invoice_custom_line_no': isLastInvoiceCustomLineNo + 1,
                            'copy_history_flag': line.copy_history_flag,
                            'product_tax_category': get_product_tax_category(line, line)
                        })]
                        isLastInvoiceCustomLineNo += 1
        elif self.copy_history_from == 'multi_remove':
            remove_ids = self.copy_history_item.split(',')
            remove_lines = []
            for remove_id in remove_ids:
                for line in self.invoice_line_ids:
                    if line.invoice_custom_line_no == int(remove_id):
                        remove_lines.append(line)
            lines_to_keep = self.invoice_line_ids.filtered(lambda line: line not in remove_lines)
            self.invoice_line_ids = lines_to_keep
            self._onchange_invoice_line_ids()
        elif self.copy_history_from == 'add_product_custom':
            self.invoice_line_ids = [(0, False, {
                'product_barcode': ' ',
                'invoice_custom_line_no': isLastInvoiceCustomLineNo + 1,
            })]
    ...
    # LongDN MOD START 2022/04/27
    # Call it when add new line no, to get variable orderedBy from client
    # Before control orderedBy to reset line no
    @api.model
    def triggered_control_line_no_follow_ordered_by(self, *args):
        global CONTROL_LINE_NO_FOLLOW_ORDER_BY
        # if len(args) > 0:
        #     for arg in args:
        #         if arg['name'] != 'invoice_custom_line_no':
        #             CONTROL_LINE_NO_FOLLOW_ORDER_BY = 'doNotNeedResetLineNo'
        #             break
        #         elif arg['name'] == 'invoice_custom_line_no' and arg['asc']:
        #             CONTROL_LINE_NO_FOLLOW_ORDER_BY = 'isResetLineNoFollowAsc'
        #         elif arg['name'] == 'invoice_custom_line_no' and not arg['asc']:
        #             CONTROL_LINE_NO_FOLLOW_ORDER_BY = 'isResetLineNoFollowDesc'
        CONTROL_LINE_NO_FOLLOW_ORDER_BY = 'isResetLineNoFollowAsc'
        return CONTROL_LINE_NO_FOLLOW_ORDER_BY
    # LongDN MOD END 2022/04/27
    @api.model
    def set_cost_error(self, *args):
        if len(args) > 0:
            cost_error_line = get_session('cost_error_line')
            item = {}
            item['value'] = True
            cost_error_line[args[0]['invoice_custom_line_no']] = item
            write_session('cost_error_line', cost_error_line)
        return True

    @api.model
    def set_cost_ok(self, *args):
        if len(args)>0:
            cost_error_line = get_session('cost_error_line')
            item = {}
            item['value'] = False
    ...
    @api.onchange('x_studio_business_partner', 'x_studio_name', 'ref', 'x_bussiness_partner_name_2',
                  'x_studio_address_1',
                  'x_studio_address_2', 'x_studio_address_3', 'x_studio_summary', 'sales_rep', 'partner_department')
    def _check_flag_history(self):
        global partner_id_for_price_history
        for rec in self:
            rec.profit_management_flg = self.env.company.profit_management_flg
            if rec.x_studio_name or rec.x_studio_business_partner or rec.ref or rec.x_bussiness_partner_name_2 or rec.x_studio_address_1 \
                    or rec.x_studio_address_2 or rec.x_studio_address_3 or rec.x_studio_summary or rec.sales_rep or rec.partner_department:
                rec.flag_history = 1
                partner_id_for_price_history = rec.x_studio_business_partner.id
                self.env['ir.config_parameter'].sudo().set_param('global.account_move_partner_id',rec.x_studio_business_partner.id)
            else:
                rec.flag_history = 0
    ...
    def _compute_fill_data_with_quotation(self):
        ...
        for rec in self:
            if rec.trigger_quotation_history:
                ...
                rec.x_studio_business_partner = rec.trigger_quotation_history.partner_id
                # Long MOD START 2022-05-19
                rec.sales_rep = rec.trigger_quotation_history.cb_partner_sales_rep_id
                rec.partner_department = rec.trigger_quotation_history.partner_department
                # Long MOD END 2022-05-19
                # Call method to set data when change partner
                copy_data_from_history_quotation_partner(rec, rec.x_studio_business_partner, rec.trigger_quotation_history)
    ...
    def _compute_amount(self):
        ...
        for move in self:
                    ...
                    if not line.exclude_from_invoice_tab:
                        ...
                        if line.x_invoicelinetype != 'サンプル':
                            if move.x_voucher_tax_transfer == 'voucher' \
                                    and line.product_tax_category == 'foreign':
                            ...
                            total_cost += line.invoice_custom_lineamount_cost
            ...
            if move.x_voucher_tax_transfer != 'custom_tax':
                if move.x_voucher_tax_transfer == 'voucher':
                    if move.invoice_line_ids:
                        move.x_voucher_tax_amount = move.calc_all_total(['10.0', '8.0'], True)['total_tax']
                    else:
                        move.x_voucher_tax_amount = 0
                else:
            ...
            move.amount_cost = total_cost
            move.amount_untaxed = total_untaxed_custom
            move.amount_gross_profit= move.amount_untaxed - move.amount_cost
            if move.amount_untaxed!=0:
                move.gross_profit_rate = move.amount_gross_profit/move.amount_untaxed
            else:
                move.gross_profit_rate = 0
    ...
    def compute_amount_type_voucher(self):
        invoices = self.env['account.move'].search([('x_voucher_tax_transfer', '=', 'voucher')])
        for rec in invoices:
            if rec.customer_tax_rounding != rec.partner_id.customer_tax_rounding:
                self._cr.execute("UPDATE account_move SET customer_tax_rounding = %s WHERE id = %s", (rec.partner_id.customer_tax_rounding, rec.id))
                logger.info('UPDATE customer_tax_rounding account_move of invoice: ' + rec.x_studio_document_no)
                logger.error('UPDATE customer_tax_rounding account_move of invoice: ' + rec.x_studio_document_no)
                rec.customer_tax_rounding = rec.partner_id.customer_tax_rounding
            result = rec.calc_all_total(['10.0', '8.0'], True)
            self._cr.execute("UPDATE account_move SET amount_tax = %s, x_voucher_tax_amount = %s, amount_total = %s WHERE id = %s", (result['total_tax'], result['total_tax'], result['total_grand'], rec.id))
    ...
    def _onchange_x_test(self):
        ...
        for voucher in self:
            ...
            for l in voucher.x_history_voucher.invoice_line_ids.sorted(lambda i: i.invoice_custom_line_no):
                ...
                line_data['product_tax_category'] = get_product_tax_category(l, l)
            ...
            if voucher.x_history_voucher._origin.id:
                ...
                voucher.sales_rep = voucher.x_history_voucher.sales_rep
                voucher.delivery_id = voucher.x_history_voucher.delivery_id
                voucher.delivery_name = voucher.x_history_voucher.delivery_name
                voucher.delivery_address = voucher.x_history_voucher.delivery_address
                # voucher.x_studio_cus_salesslipforms_table = voucher.x_history_voucher.x_studio_cus_salesslipforms_table
                # LONG MOD START 2022/04/21 - FIX DATE IS NOT CHANGED
                ...
                if (voucher.x_history_voucher.x_voucher_tax_transfer and
                    voucher.x_history_voucher.x_voucher_tax_transfer != 'foreign_tax'):
                    voucher.x_voucher_tax_transfer = voucher.x_history_voucher.x_voucher_tax_transfer
                else:
                    voucher.x_voucher_tax_transfer = 'voucher'
                # MOD - LIEM - 20230929 - END
                voucher.customer_tax_rounding = voucher.x_history_voucher.customer_tax_rounding
                voucher.partner_department = voucher.x_history_voucher.partner_department
    ...
    def _onchange_x_studio_date_invoiced(self):
        ...
        # ADD - LIEM - 20220105 - START
        # ===============================
        if self.check_date_less_than_bill_date(self.x_studio_business_partner, self.x_studio_date_invoiced):
            return {
                'warning': {
                    'title': 'アラート',
                    'message': '請求締切以前の日付です。'}
        ...
        # Start Del 23/0809 TrungTM
        # # Start Add by TMTrung for change price unit according to customer_code
        # for line in self.invoice_line_ids:
        #     line.get_price(line.product_barcode \
        #                    , self.partner_id.customer_code \
        #                    , self.x_studio_date_invoiced \
        #                    , self.x_voucher_tax_transfer \
        #                    , line.tax_rate
        #                    , line)
        # self._compute_amount()
        # # End Del 23/0809 TrungTM
        # End Add by TMTrung for change price unit according to customer_code
    ...
    def _get_detail_business_partner(self):
        ...
        # Start Del TrungTM 230808 (https://vjppd.backlog.jp/view/CDS-67#comment-1409828743)
        # # Start Add by TMTrung for change price unit according to customer_code
        # for line in self.invoice_line_ids:
        #     line.get_price(line.product_barcode \
        #                    , self.partner_id.customer_code \
        ...
        #
        # # End Add by TMTrung for change price unit according to customer_code
        # End Del TrungTM 230808 (https://vjppd.backlog.jp/view/CDS-67#comment-1409828743)
    ...
    def _onchange_invoice_line_ids(self):
        ...
        if len(self.invoice_line_ids):
            for index, invoice_line_id in enumerate(self.invoice_line_ids):
                old_line_no = invoice_line_id.invoice_custom_line_no
                new_line_no = index + 1
                invoice_line_id.invoice_custom_line_no = new_line_no
                if cost_error_line and len(cost_error_line) > 0 \
                    and old_line_no in cost_error_line.keys() \
                    and new_line_no != old_line_no:
                    cost_error_line[new_line_no] = cost_error_line.pop(old_line_no)

                if new_max_line_no < new_line_no:
                    new_max_line_no = new_line_no

            # when last delete row ==> remove lastest item of cost_error_line
            if cost_error_line and len(cost_error_line)>0:
                old_max_line_no = max(cost_error_line.keys())
            if old_max_line_no > new_max_line_no:
                cost_error_line.pop(old_max_line_no)
        else:
            cost_error_line = {}
    ...
    def onchange_delivery(self):
        if self.delivery_id:
            self.delivery_code = self.delivery_id.delivery_code
            self.delivery_name = self.delivery_id.name
            self.delivery_address = self.delivery_id.street
        else:
            self.delivery_code = ""
            self.delivery_name = ""
    ...
    def _validate_data(self,vals):
        is_error_cost_price = False
        cost_error_line = get_session('cost_error_line')
        for key in cost_error_line:
            if key:
                if cost_error_line[key]['value']:
                    is_error_cost_price = True
                    break
        if is_error_cost_price:
            raise UserError('原価は必須な項目です。')
    ...
    def write(self, vals):
        ...
        if module_context.get('form_view_initial_mode') == 'edit':
            if self.id > 0:
                ...
                bill_invoice_ids = self.env['bill.invoice'].search([('account_move_id', '=', self.id)])
                if bill_invoice_ids:
                    raise UserError('売上伝票が請求処理されているため、変更はできません。')
                self._validate_data(vals)
        result = super(ClassInvoiceCustom, self).write(vals)
        write_session('cost_error_line', {})
        return result
                ...
                if tax_key_add_base not in done_taxes:
                    if line.currency_id != self.company_id.currency_id:
                        amount = self.company_id.currency_id._convert(line.tax_base_amount, line.currency_id,
                                                                      self.company_id,
                                                                      line.date or datetime.now().astimezone(
...
class AccountMoveLine(models.Model):
    ...
    supplier_code = fields.Many2one('res.supplier', string='仕入先コード')
    supplier_name = fields.Char('Supplier name', related='supplier_code.name')
    ...
    date = fields.Date(related='move_id.x_studio_date_invoiced', store=True, readonly=True, index=True, copy=False,
                       group_operator='min')
    line_no_input = fields.Float(default='', digits=(_, 0))
    invoice_custom_line_no = fields.Integer('Line No', default=get_default_line_no)
    # invoice_custom_line_no = fields.Integer('Line No')
    ...
    # Start Add for amount cost TrungTM 23.04.23
    invoice_custom_lineamount_cost = fields.Float('Line Amount Cost', compute='compute_line_amount')
    # End Add for amount cost TrungTM 23.04.23
    ...
    # Start TMTrung Add makercd for history search
    product_maker_cd = fields.Char('product_maker_cd', compute='compute_product_maker_cd')
    def compute_product_maker_cd(self):
        for line in self:
            line.product_maker_cd = line.product_id.product_custom_freight_category.search_key_freight

    # End TMTrung Add makercd for history search
    ...
    product_tax_category = fields.Char(string="Product tax category", store=True)
    ...
    is_required_cost = fields.Boolean(default=False, store=False, compute='set_required_cost_flag')
    def set_required_cost_flag(self):
        for line in self:
            line.is_required_cost = self.env.company.input_cost_required_flg

    # def _compute_customer_product_price_history(self):
    #     for record in self:
    #         record.supplier_code = record.product_id.supplier_code.supplier_code
    #         record.supplier_name = record.product_id.supplier_code.name
    ...
    def change_tax_rounding(self):
        ...
        self.is_required_cost =  self.env.company.input_cost_required_flg
    ...
    def _onchange_product_code(self):
        if len(self.changed_fields) > 20:
            del self.changed_fields[0:2]
        global product_barcode_for_price_history
        if 'product_code'+str(self.id) not in self.changed_fields:
            if self.product_code:
                ...
                # if product:
                if len(product) == 1:
                    self.changed_fields.append('product_barcode'+str(self.id))
                    self.changed_fields.append('invoice_custom_standardnumber'+str(self.id))
                    self.product_id = product.id
                    self.product_barcode = product.barcode
                    product_barcode_for_price_history = product.barcode
                    ...
                    # start upd TrungTM 23.03.19 for get price
                    self.get_price(product.barcode \
                                   , self.move_id.partner_id.customer_code \
                                   , self.move_id.x_studio_date_invoiced \
                                   , self.move_id.x_voucher_tax_transfer \
                    ...
                    # end upd TrungTM 23.03.19 for get price
                    return
            ...
            self.changed_fields.remove('product_code'+str(self.id))
    ...
    def get_price(self, \
        ...
        if not par_customer_code:
            par_customer_code = ''
        if not par_date:
            par_date = datetime.today()
        if not par_get_tax_type:
            par_get_tax_type = ''
        if not par_tax_rate:
            par_tax_rate = -1

        query = "SELECT * from get_price('{}','{}','{}','{}',{})".format( \
            par_customer_code \
            , par_jan_code \
            , par_date.strftime('%Y%m%d') \
            , par_get_tax_type \
            , par_tax_rate)
        ...
        for record in query_res:
            out_par_line.price_include_tax = record["return_price_include_tax"]
            out_par_line.price_no_tax = record["return_price_no_tax"]
            out_par_line.price_unit = record["return_price"]
            out_par_line.product_standard_price = record["return_standard_price"]
            out_par_line.x_product_cost_price= record["return_cost"]
    ...
    def _onchange_product_barcode(self):
        if len(self.changed_fields) > 20:
            del self.changed_fields[0:2]
        global product_barcode_for_price_history
        if 'product_barcode'+str(self.id) not in self.changed_fields:
            if self.product_barcode:
                ...
                if product:
                    self.changed_fields.append('product_code'+str(self.id))
                    self.changed_fields.append('invoice_custom_standardnumber'+str(self.id))
                    self.product_id = product.id
                    self.invoice_custom_standardnumber = product.product_custom_standardnumber
                    self.supplier_code = product.supplier_code

                    product_barcode_for_price_history = self.product_barcode
                    ...
                    # start upd TrungTM 23.03.19 for get price
                    self.get_price(self.product_barcode  \
                                   , self.move_id.partner_id.customer_code \
                                   , self.move_id.x_studio_date_invoiced \
                                   , self.move_id.x_voucher_tax_transfer \
                                   , product.product_tax_rate
            ...
            self.changed_fields.remove('product_barcode'+str(self.id))
    ...
    def _onchange_invoice_custom_standardnumber(self):
        if len(self.changed_fields) > 20:
            del self.changed_fields[0:2]
        global product_barcode_for_price_history
        if 'invoice_custom_standardnumber'+str(self.id) not in self.changed_fields:
            ...
            self.changed_fields.remove('invoice_custom_standardnumber'+str(self.id))
        ...
        if 'product_id' not in self.changed_fields:
            for line in self:
                if not line.product_id or line.display_type in ('line_section', 'line_note'):
                    line.name = ''
                    line.product_name = ''
                    line.product_tax_category = False
                ...
                # start del TMTrung 23.03.18 for new getting price method
                # line.product_standard_price = line.product_id.standard_price
                # line.x_product_cost_price = line.product_id.cost
                # end del TMTrung 23.03.18 for new getting price method
                ...
                line.product_tax_category = line.product_id.product_tax_category
    ...
    def get_compute_lineamount_cost(self, x_product_cost_price, quantity, type_rounding):
        if x_product_cost_price is None:
            x_product_cost_price = 0
        if quantity is None:
            quantity = 0
        result = x_product_cost_price * quantity
        # TODO recounting rounding
        return rounding(result, 0, type_rounding)
    ...
    def compute_line_amount(self):
        for line in self:
            line.invoice_custom_lineamount = self.get_compute_lineamount(line.price_unit, line.discount, line.quantity,
                                                                         line.move_id.customer_tax_rounding)
            line.invoice_custom_lineamount_cost = self.get_compute_lineamount_cost(line.x_product_cost_price, line.quantity,
                                                                                line.move_id.customer_tax_rounding)
    @api.depends('move_id.x_voucher_tax_transfer', 'move_id.customer_tax_rounding',
    ...
    def _onchange_price_subtotal(self):
        for line in self:
            ...
            line.invoice_custom_lineamount = self.get_compute_lineamount(line.price_unit, line.discount,
                                                                         line.quantity,
                                                                         line.move_id.customer_tax_rounding)
            line.invoice_custom_lineamount_cost = self.get_compute_lineamount_cost(line.price_unit,
                                                                                     line.quantity,
                                                                                     line.move_id.customer_tax_rounding)
            if (line.move_id.x_voucher_tax_transfer == 'foreign_tax'
                and line.product_id.product_tax_category != 'exempt') \
    ...
    def _onchange_price_unit(self):
        ...
        if self.product_id.product_tax_category == 'foreign':
            ...
            self.price_no_tax = self.price_include_tax = self.price_unit
        # TH - done
        #---old source
        # # TH - code
        # if self.product_id.product_tax_category == 'foreign':
    ...
    # Search function of AccountMoveLine(models.Model):
    @api.model
    def search(self, args, offset=0, limit=None, order=None, count=False):
        global partner_id_for_price_history
        """
        ...
        if ctx.get('have_advance_search'):
            ...
            arr = ["partner_id", "partner_id.name", "move_id.x_userinput_id", "customer_state", "customer_group",
                   ...
                   "invoice_custom_standardnumber","product_maker_cd", "product_maker_name", "product_name"]
            for se in args:
                if se[0] == '&':
                    continue
                if se[0] == 'search_category' and se[2] == 'equal':
                ...
                if se[0] == 'move_id.x_studio_document_no':
                    ...
                    if len(se[2]) < 11:
                        se[2] = ''.join(["ARI-", string_middle, se[2]])
                # TH - done
                # Start TrungTM add MakerCD for adv search
                if se[0] == 'product_maker_cd':
                    se[0] = 'product_id.product_custom_freight_category.search_key_freight'
                # End TrungTM add MakerCD for adv search
            args = domain

        # ADD - LIEM - 20220122 - START
        args.append(['company_id', '=', self.env.company.id])

        # Start Del 230809
        # args.append(['product_barcode', '!=', False])
        # End Del 230809

        args.append(['move_id.type', '=', 'out_invoice'])
        # ADD - LIEM - 20220122 - END

        if ctx.get('price_history'):
            domain = []
            for itm in args:
            #     if itm[0] == 'product_barcode' and itm[2]:
            #         product_barcode_for_price_history =  itm[2]
            # if partner_id_for_price_history and product_barcode_for_price_history :
                domain += [['partner_id', '=',int(self.env['ir.config_parameter'].sudo().get_param('global.account_move_partner_id'))]]
                # domain += [('product_barcode', '=', product_barcode_for_price_history)]
            args += domain
    ...
    @staticmethod
    def get_product_tax_category(line, line_master):
        return get_product_tax_category(line, line_master)
...
# class NewClassSupplierCustom(models.Model):
#     _inherit = 'res.supplier'
#
#     def _get_name(self):
#         name = '%s%s' % (self.supplier_code and '%s - ' % self.supplier_code or '', self.name)
#         return name.strip()
#
#     def name_get(self):
#         result = []
#         # sale_order = self.env['res.supplier']
#         for rec in self:
#             name = '%s%s' % (rec.supplier_code and '%s - ' % rec.supplier_code or '', rec.name)
